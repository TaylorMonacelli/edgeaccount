...
lxc init ubuntu:focal t19 --config=user.user-data="$(cat /tmp/my-user-data-t19)"

for count in {1..3}; do
    echo boot count $count

    lxc start t19
    lxc shell t19 -- bash -c 'cloud-init status --wait'
    lxc shell t19 -- bash -c 'cat /var/tmp/hello-runcmd.txt'
    lxc shell t19 -- bash -c 'cat /var/tmp/hello-per-boot.txt'
    lxc shell t19 -- bash -c 'cat /var/tmp/hello-per-once.txt'
    lxc stop t19
done
