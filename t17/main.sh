...
lxc init ubuntu:focal t17 --config=user.user-data="$(cat /tmp/my-user-data-t17)"

for count in {1..3}; do
    echo boot count $count

    lxc start t17
    lxc shell t17 -- bash -c 'cloud-init status --wait'
    lxc shell t17 -- bash -c 'cat /var/tmp/hello-runcmd.txt'
    lxc shell t17 -- bash -c 'cat /var/tmp/hello-per-boot.txt'
    lxc shell t17 -- bash -c 'cat /var/tmp/hello-per-once.txt'
    lxc stop t17
done
